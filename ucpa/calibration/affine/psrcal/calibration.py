...
class AffineCal(LBFGS_Objective):
    ...
    def __init__(self, num_classes, bias=True, scale=True, priors=None):    
        ...
        if bias:
            if self.priors is not None:
                ...
                self.bias = Parameter(torch.zeros(num_classes, dtype=torch.float64))
        else:
            self.bias = 0

    def train(self, scores, labels, quiet=True):
        self.scores = scores
        self.labels = labels
