...
def plot_metric_vs_samples(
    ...
    results = pd.read_csv(f"results/{experiment}/calibration_scores.csv")
    dfs_result = []
    if plot_original:
    ...
    fig.tight_layout()
    fig.savefig(f"results/{experiment}/shots_vs_metric.png")
...
def plot_metric_vs_nshots(
    ...
    fig.tight_layout()
    fig.savefig(f"results/{experiment}/samples_vs_metric.png")
